#include "stdafx.h"
#include "PlayerWallCollision.h"
#include "Tilemap.h"

#define TILESIZE 40

bool PlayerWallCollision::verticalCollision(sf::FloatRect& hitbox, int x, int y, int& tileCoordX)
{
	/*
	* Checks collision on x-axis
	* Updates the co-ordinate of tile (which is colliding with PlayerWallCollision)
	*/
...
bool PlayerWallCollision::collisionHorizontalUp(sf::FloatRect& hitbox, int x, int y, int& tileCoordY)
{
...
bool PlayerWallCollision::collisionHorizontalDown(sf::FloatRect& hitbox, int x, int y, int& tileCoordY)
{
	int tileXpixels = x - (x % TILESIZE);	  //calculate the x position (pixels!) of the tiles we check against
...
bool PlayerWallCollision::collisionSlope(sf::FloatRect& hitbox, int sx, int sy, int& tsx, int& tsy)
{
	/*
	* Checks tile containing the PlayerWallCollision is sloped tile
	* If tile is sloped, adjust y co-ordinate of PlayerWallCollision according to x
	*/
